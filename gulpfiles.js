...
gulp.task('css',function(){
    ...
    return gulp.src( '../jinfu/sass/*.scss')
        .pipe(sass().on('error',sass.logError))
        .pipe(postcss(processors))
        .pipe(gulp.dest( '../jinfu/css'))
        .pipe(connect.reload());
});

//定义一个html任务
gulp.task('html',function(){
    gulp.src('../jinfu/staticHtml/**/*.html')
        .pipe(connect.reload());
});
...
gulp.task('watch', function() {
    gulp.watch('../jinfu/sass/*.scss', ['css']);
    //gulp.watch('../jinfu/less/!*.less', ['less']);
    //gulp.watch('../jinfu/css/!*.css', ['testAutoFx']);
    gulp.watch('../jinfu/staticHtml/!**/!*.html', ['html']);
})
